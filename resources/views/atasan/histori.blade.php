    ...
    <div class="max-w-6xl mx-auto bg-white shadow-2xl rounded-2xl p-6 md:p-10">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-8">
            ...
            <form method="GET" action="{{ route('atasan.histori') }}" class="w-full md:w-auto grid grid-cols-1 md:grid-cols-5 gap-4">
                <div>
                    <input type="text" id="nama-pegawai" name="nama" value="{{ request('nama') }}" placeholder="Nama Pegawai..." 
                ...
                <tbody>
                    @forelse($histori as $izin)
                        <tr class="hover:bg-blue-50 border-b">
                            <td class="px-6 py-4">{{ $izin->pegawai->nama }}</td>
                            <td class="px-6 py-4">{{ $izin->tanggal_pengajuan->format('d-m-Y') }}</td>
                            <td class="px-6 py-4">{{ $izin->tanggal_izin }}</td>
...
$('#tahun-select, #bulan-select').on('change', function () {
                ...
                success: function(data) {
                    ...
                    if (data.length > 0) {
                        data.forEach(function(pegawai) {
                            pegawaiList += '<div class="px-4 py-2 hover:bg-blue-100 cursor-pointer text-sm" data-nama="'+pegawai.text+'">'+pegawai.text+'</div>';
                        });
                    } else {
                        pegawaiList = '<div class="px-4 py-2 text-gray-400 text-sm">Tidak ditemukan</div>';
                    }
                    $('#list-pegawai').html(pegawaiList).removeClass('hidden');
                }
            });
        } else {
    ...
    $(document).on('click', '#list-pegawai div', function() {
        $('#nama-pegawai').val($(this).data('nama'));
        $('#list-pegawai').addClass('hidden');
    });
